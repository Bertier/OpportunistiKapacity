...
for times,id_nodes,posx_nodes,posy_nodes in DatasetReader(dataset):
    time=float(times[0])
    ...
    for edge in instant_edges - old_edges:
        ...
        ya = 0
        active_contacts_data[edge] = integrate(ya,yb)
        active_contacts_start[edge] = time - time_granularity
    ...
    for edge in old_edges - instant_edges:
        ...
        terminated_contacts[final_edge_key] = active_contacts_data[edge] + integrate(ya,yb)
        del active_contacts_data[edge]
        del active_contacts_start[edge]
    
    old_edges = instant_edges
    old_graph = instant_goodput
    print time, len(active_contacts_data), len(terminated_contacts)
os.mkdir(output_dir) 
with open(output_dir + 'contacts_data_nearby_%s_%s_%s.json' % (dataset,
                                                               propagation_name,
                                                               modulation_name),
          'w') as fp:
