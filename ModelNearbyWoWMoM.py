...
def twoRay_loss(distance, epsilon_r=1.00673130,height=1.39):
    ...
    d_reflection=np.sqrt( (distance**2) + (height_sender + height_receiver)**2)
    ...
    gamma= (sin_theta - np.sqrt(epsilon_r - cos_theta**2)) / (sin_theta + np.sqrt(epsilon_r - cos_theta **2))
    LossTwoRay=10*np.log10((4 * np.pi * (distance / wavelength) * 1 / (np.sqrt(((1 + gamma * np.cos(phi))**2 + gamma**2 * np.sin(phi)**2))))**2)
...
def Wifi5_stepwise_fit(rssi):
    threshold_rssi=np.array([0,-55,-57,-58,-59,-63,-67,-70,-72,-75,-min_rssi])
    data_rate=np.array([22.894102,24.934630,24.119049,24.727494,23.782886,20.275357,17.555846,14.414938,8.225040,3.574197])*8
    if rssi >= 0:
        return data_rate[0]
    elif rssi <= threshold_rssi[-1]:
        return 0
