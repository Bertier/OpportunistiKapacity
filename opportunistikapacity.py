...
class ContactTrace(object):
    def __init__(self, dataset, propagation, modulation,data_kind):
        ...
        try:
            ...
            sys.exit(6)
        self.sampling_granularity=np.absolute(eval(self.presampled_contacts.keys()[0]))
        #Simply unpack the elements from the sampled datasets found in the json file.
        self.contact_time_to_throughput_probability={}
        for time_key in self.presampled_contacts:
            self.contact_time_to_throughput_probability[time_key]=zip(*self.presampled_contacts[time_key])
        self.number_samples=len(self.contact_time_to_throughput_probability)
        
    def get_capacity(self):
        terminated_contacts={}
        for id1,id2,time_start_raw,time_end_raw in ContactReader(self.dataset):
            #todo: take time format in consideration. For now just make the difference.
